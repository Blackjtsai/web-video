---
description: 旅遊簡報完整工作流程（美玲觸發）：Phase 1 讀素材 → Phase 2 網頁版 → Phase 3 手機版 → Phase 4 TTS → Phase 5 交付
---
...
詳細組件規範見 [`.claude/commands/references/MOBILE-CRAFT.md`](references/MOBILE-CRAFT.md)。
...
| src/components/MobilePage.tsx | SEGMENTS 陣列 |
| src/components/SplitEnding.tsx | MAPS 陣列 |
| public/audio/<id>/<N>.mp3 | 口播音頻 |

## 主題色
--surface / --accent / --text 實際值
...
```
> ⚠️ MapBtn 用英文搜尋字串，不要用中文或日文地址。

### 3. FAB 長按 Scrubber
- `touch-action: none` 在 FAB 按鈕上
- 移動超過 8px → 取消長按計時
- 長按 500ms → 開啟 Scrubber + 震動 `navigator.vibrate(40)`

### 4. 播放鎖定滑動（Scroll Lock）
正確做法：`touchmove` 監聽器 + ref（勿用透明 overlay，iOS Safari bug）
```tsx
...
### 5. 視窗等比例縮放 CSS（取代 1920px transform scale）
```css
.stage-frame {
...
字體全用 `clamp()`：
```css
.dX-day-title { font-size: clamp(32px, 3.6vw, 48px); }
```
...
- [ ] 最後一步 `↓` 彈出 SplitEnding？PDF 按鈕 / 地圖連結正常？

### 手機版驗收
- [ ] 封面圖等比例顯示，無裁切、無漸層 overlay？
- [ ] 播放時無法手動捲動（touchmove 被攔截）？
- [ ] 暫停後可立刻自由捲動？
- [ ] FAB 播放 / 長按 Scrubber 正常？
- [ ] 地圖按鈕點開 Google Maps 正確？
- [ ] LINE 開啟自動跳出外部瀏覽器？
- [ ] LINE 分享預覽卡有圖、有標題？（og:image 用橫幅圖）
...
| 文件 | 時機 |
|---|---|
| [`.claude/commands/references/MOBILE-CRAFT.md`](references/MOBILE-CRAFT.md) | Phase 3 必讀：完整 FAB + Scroll Lock 規範 |
| `~/.claude/skills/web-video-presentation/` | Phase 2 scaffold 腳本 |
