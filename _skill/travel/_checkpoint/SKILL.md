...
找到 pattern 後，**必須先判斷該放在哪裡**，再決定怎麼寫：

### 放在哪的判斷規則

問自己一個問題：**「每次建新專案都要做這件事嗎？」**

| 答案 | 放在哪 | 範例 |
|------|--------|------|
| ✅ 是，每次都要做 | **Phase 步驟流程內**（2.1、3.1 等），寫成操作步驟或 code snippet | LINE script 補 index.html、scaffold 後刪範例章節 |
| ⚠️ 踩過坑但不是每次都觸發 | **Phase 步驟旁的 ⚠️ 警告**，或驗收清單的 `[ ]` 項目 | 某章節圖片路徑錯誤、split.css 衝突 |
| 📚 參考用、偶爾查 | **「重要技術細節」章節**，不放進流程 | CSS 架構原理、token 說明 |

> ⚠️ **陷阱**：知識放在「重要技術細節」但沒進 Phase 流程 → 建新專案時不會看到 → 下次還會犯同樣錯誤。
> 澎湖記過 LINE script，但放在技術細節章節，仙台和北海道建立時跳過了，同樣錯誤發生兩次。

若有新內容，依上方判斷補充到 `_skill/travel/SKILL.md` 的對應位置。
