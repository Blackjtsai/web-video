...
| [20261220仙台](site/20261220仙台/CLAUDE.md) | 完成 | 7 章 32 步 |
| [20261006北海道八日遊](site/20261006北海道八日遊/blueprint.md) | 進行中（TTS 待合成） | 10 章 41 步 |
| [新疆精選旅遊路線](site/新疆精選旅遊路線/blueprint.md) | 完成（純手機版） | 18 段語音 · Google Maps |
| [20270228志賀高原滑雪](site/20270228志賀高原滑雪/blueprint.md) | 完成 | 8 章 32 步 |
| [20270228斑尾高原滑雪](site/20270228斑尾高原滑雪/blueprint.md) | 完成（網頁版＋手機版） | 6 章 12 步（網頁）・12 段語音（手機）・Wikimedia 實景照 |
...
**新增或修改「卡片型」內容（住宿、地圖導航、PDF 下載按鈕等）時，網頁版 + 手機版 + 結尾頁三處必須同步檢查，不能改完一處就回報完成。**

每個旅遊專案都是「網頁版章節（Day*.tsx）／手機版（MobilePage.tsx）／結尾頁（SplitEnding.tsx）」三套 UI 各自維護同一份資訊，沒有共用元件強制同步，容易改了一處漏另一處而不自知。
**執行規則**：改完後列出這三個檔案逐一確認同一功能是否都有對應實作，再回報「完成」；有把握判斷「這次改動不涉及卡片內容」（例如純文字修正、CSS 調整）才可以跳過。
發現脈絡：2026-08-21 於志賀高原滑雪專案新增訂房 PDF 下載與地圖導航按鈕，只改了網頁版 Day1 + 結尾頁，手機版 MobilePage 的「地圖導航」整組區塊（8 個雪場錨點）漏掉未同步，直到使用者自己發現才補上。
